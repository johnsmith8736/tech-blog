...
export default function PostCard({ post }: PostCardProps) {
    ...
    return (
        <article className="group relative overflow-hidden rounded-[1.35rem] border border-white/10 bg-white/[0.035] shadow-[0_16px_45px_rgba(0,0,0,0.16)] transition-all duration-300 hover:-translate-y-1 hover:border-cyan-200/30 hover:bg-white/[0.05]">
            <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(147,197,253,0.12),transparent_32%),linear-gradient(120deg,rgba(255,255,255,0.04),transparent_40%,rgba(209,180,111,0.05))] opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
            <div className="pointer-events-none absolute inset-y-0 left-0 w-px bg-gradient-to-b from-cyan-200/0 via-cyan-200/70 to-amber-200/0" />
            ...
            <Link href={`/posts/${post.slug}`} className="relative block p-5 md:p-6">
                <div className="flex flex-col gap-5 md:flex-row md:items-start md:gap-8">
                    <div className="flex items-center justify-between gap-3 md:w-40 md:flex-col md:items-start md:justify-start md:gap-2">
                        ...
                        <span className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[10px] font-mono uppercase tracking-[0.18em] text-slate-200">
                            Article
                        </span>
                    </div>
                    ...
                    <div className="min-w-0 flex-1 space-y-4">
                        <div className="flex flex-wrap items-center gap-2">
                            <span className="rounded-full border border-cyan-200/20 bg-cyan-200/[0.06] px-2.5 py-1 text-[10px] font-mono uppercase tracking-[0.18em] text-cyan-100">
                                {displayCategory}
                            </span>
                            ...
                            {post.tags && post.tags.length > 0 && post.tags.slice(0, 3).map((tag) => (
                                <span
                                    key={tag}
                                    className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[10px] font-mono uppercase tracking-[0.16em] text-slate-300 transition-colors group-hover:border-white/20 group-hover:text-white"
                                >
                                    #{tag}
                                </span>
                            ))}
